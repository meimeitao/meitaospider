...
casper.then(function() {
    ...
    var retColor = this.evaluate(function() {
        ...
        var colorOptions = document.querySelectorAll(".color .swatchanchor");
        var color = {};
        ...
        for (var i = 0; i < colorOptions.length; i++) {
            ...
            var thumbmail = tmpOption.querySelector("img.swatchimage");
            var tmpDesc = tmpOption.title;
            var tmpSample = thumbmail ? thumbmail.src : "";
            var tmpDemo = "";
    ...
    var retSize = this.evaluate(function() {
        ...
        var sizeOptions = document.querySelectorAll("."+sizeID+" select option");
    ...
    for (var x in stocks) {
        ...
        for (var m in tmpStock) {
            ...
            this.evaluate(function setProperties(id, value) {
                if (id == "color") {
                    var ele = document.querySelector("."+id+" a[title='"+value+"']");
        ...
        stockValue = this.evaluate(function(size) {
            var ele = document.querySelector("option[value='"+size+"']");
            return ele.disabled;
        }, tmpSize);
        if (stockValue) {
            stocks[x].soldout = 1;
